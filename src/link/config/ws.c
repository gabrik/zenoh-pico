...
void _z_ws_config_onto_str(char *dst, const _z_str_intmap_t *s) {
    WS_CONFIG_MAPPING_BUILD
...
_z_str_intmap_result_t _z_ws_config_from_strn(const char *s, size_t n) {
    WS_CONFIG_MAPPING_BUILD

    return _z_str_intmap_from_strn(s, argc, args, n);
}

_z_str_intmap_result_t _z_ws_config_from_str(const char *s) { return _z_ws_config_from_strn(s, strlen(s)); }
#endif
